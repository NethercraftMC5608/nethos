#!/bin/bash
# The thread-exit poll hang, minimally. Seconds per iteration, no disk.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT="$ROOT/kernel/ldk/build/exitpoll.cpio"
mkdir -p "$(dirname "$OUT")"
...
docker run --rm --platform linux/arm64 -v "$ROOT/kernel/init:/src:ro" nethos-ldk sh -ec '
    ...
    gcc -fPIE -pie -O2 -pthread /src/exitpoll.c -o /tmp/root/nk-init
