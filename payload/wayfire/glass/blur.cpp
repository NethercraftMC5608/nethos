...
using blur_algorithm_provider =
    std::function<nonstd::observer_ptr<nethos_glass_base>()>;
...
static int calculate_damage_padding(const wf::render_target_t& target, int blur_radius)
{
    ...
    // Refraction is measured in logical pixels, unlike the Kawase radius.
    return std::ceil(blur_radius / scale) + 16;
}
...
namespace scene
{
class nethos_glass_node_t : public transformer_base_node_t
{
  public:
    blur_algorithm_provider provider;
    nethos_glass_node_t(blur_algorithm_provider provider) : transformer_base_node_t(false)
    {
...
class nethos_glass_render_instance_t : public transformer_render_instance_t<nethos_glass_node_t>
{
    nethos_glass_node_t::saved_pixels_t *saved_pixels = nullptr;
...
void nethos_glass_node_t::gen_render_instances(std::vector<render_instance_uptr>& instances,
    damage_callback push_damage, wf::output_t *shown_on)
{
    auto uptr =
        std::make_unique<nethos_glass_render_instance_t>(this, push_damage, shown_on);
...
class nethos_glass : public wf::plugin_interface_t
{
  ...
  public:
    ...
    wf::view_matcher_t blur_by_default{"nethos-glass/blur_by_default"};
    wf::option_wrapper_t<std::string> method_opt{"nethos-glass/method"};
    wf::option_wrapper_t<wf::buttonbinding_t> toggle_button{"nethos-glass/toggle"};
    wf::config::option_base_t::updated_callback_t blur_method_changed;
    std::unique_ptr<nethos_glass_base> blur_algorithm;

    void add_transformer(wayfire_view view)
    {
        auto tmanager = view->get_transformed_node();
        if (tmanager->get_transformer<wf::scene::nethos_glass_node_t>())
        {
            return;
        }
        ...
        auto node = std::make_shared<wf::scene::nethos_glass_node_t>(provider);
    ...
    {
        auto tmanager = view->get_transformed_node();
        tmanager->rem_transformer<wf::scene::nethos_glass_node_t>();
    }
  ...
  public:
    ...
    void init() override
    {
        // A software compositor must remain usable. Never manufacture a hardware
        // capability merely because a render node or environment variable exists.
        if (getenv("WLR_RENDERER") && !strcmp(getenv("WLR_RENDERER"), "pixman")) return;
        OpenGL::render_begin();
        const char *renderer = reinterpret_cast<const char*>(glGetString(GL_RENDERER));
        std::string name = renderer ? renderer : "unknown";
        OpenGL::render_end();
        bool software = name.find("llvmpipe") != std::string::npos ||
            name.find("softpipe") != std::string::npos || name == "unknown";
        const char *test = getenv("NETHOS_GLASS_TEST_SOFTWARE");
        if (software && (!test || strcmp(test, "1")))
        {
            LOGI("nethos-glass: reduced effects on ", name);
            return;
        }
        LOGI("nethos-glass: backdrop refraction on ", name);
        active = true;
        wf::get_core().connect(&on_render_pass_begin);
        blur_method_changed = [=] ()
        {
            blur_algorithm = create_nethos_glass(method_opt);
        ...
        button_toggle = [=] (auto)
        {
            ...
            if (view->get_transformed_node()->get_transformer<wf::scene::nethos_glass_node_t>())
            {
                pop_transformer(view);
            } else
            {
...
DECLARE_WAYFIRE_PLUGIN(nethos_glass);
