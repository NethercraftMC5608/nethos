...
nethos_glass_base::nethos_glass_base(std::string name)
{
    this->algorithm_name = name;

    this->refraction_opt.load_option("nethos-glass/refraction");
    this->rim_opt.load_option("nethos-glass/rim");
    this->saturation_opt.load_option("nethos-glass/saturation");
    this->offset_opt.load_option("nethos-glass/" + algorithm_name + "_offset");
    this->degrade_opt.load_option("nethos-glass/" + algorithm_name + "_degrade");
    this->iterations_opt.load_option("nethos-glass/" + algorithm_name + "_iterations");
    ...
    this->refraction_opt.set_callback(options_changed);
    this->rim_opt.set_callback(options_changed);
    ...
    OpenGL::render_begin();
    blend_program.compile(blur_blend_vertex_shader, nethos_glass_fragment_shader);
    OpenGL::render_end();
}

nethos_glass_base::~nethos_glass_base()
{
    OpenGL::render_begin();
    fb[0].release();
    fb[1].release();
...
int nethos_glass_base::calculate_blur_radius()
{
    return offset_opt * degrade_opt * std::max(1, (int)iterations_opt);
}

void nethos_glass_base::render_iteration(wf::region_t blur_region,
...
wlr_box nethos_glass_base::copy_region(wf::framebuffer_t& result,
...
void nethos_glass_base::prepare_blur(const wf::render_target_t& target_fb, const wf::region_t& damage)
{
    if (damage.empty())
    {
        return;
...
void nethos_glass_base::render(wf::texture_t src_tex, wlr_box src_box, const wf::region_t& damage,
    const wf::render_target_t& background_source_fb, const wf::render_target_t& target_fb)
{
    ...
    blend_program.uniform1f("refraction", refraction_opt);
    blend_program.uniform1f("rim_strength", rim_opt);
    blend_program.uniform2f("view_size", std::max(1, src_box.width), std::max(1, src_box.height));
    blend_program.uniform2f("background_texel", 1.0f / std::max(1, fb[0].viewport_width),
        1.0f / std::max(1, fb[0].viewport_height));
...
std::unique_ptr<nethos_glass_base> create_nethos_glass(std::string)
{
    return create_nethos_kawase();
}
