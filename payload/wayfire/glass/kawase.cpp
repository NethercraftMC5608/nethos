...
class nethos_kawase_glass : public nethos_glass_base
{
  public:
    nethos_kawase_glass() : nethos_glass_base("kawase")
    {
        OpenGL::render_begin();
    ...
    int blur_fb0(const wf::region_t& blur_region, int width, int height) override
    {
        ...
        {
            sampleWidth  = std::max(1, width / (1 << i));
            sampleHeight = std::max(1, height / (1 << i));
    ...
    {
        // Include the maximum edge displacement and alpha-normal sampling footprint.
        return std::ceil(pow(2, iterations_opt + 1) * offset_opt * degrade_opt);
    }
};

std::unique_ptr<nethos_glass_base> create_nethos_kawase()
{
    return std::make_unique<nethos_kawase_glass>();
}
